...
class CacheService:
    ...
    async def set(self, key: str, value: Any, ttl: timedelta = timedelta(hours=1)):
        ...
        if self._redis_client:
            try:
                ...
                await self._redis_client.setex(key, int(ttl.total_seconds()), serialized_value)
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        memory_expired = sum(1 for item in self._memory_cache.values() if datetime.now() >= item["expires_at"])
        return {
